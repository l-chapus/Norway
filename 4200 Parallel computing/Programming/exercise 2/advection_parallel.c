...
time_step ( void )
{
    ...
    for ( int_t i=0; i<N; i++ )
        {
        for ( int_t j=0; j<local_sizes[rank]; j++ )
        {
...
border_exchange ( void )
{
    for ( int_t i=0; i<N; i++ )
        {
            MPI_Send (&(U(i,0)), 1, MPI_DOUBLE, left_neighbor, 0, MPI_COMM_WORLD);
            MPI_Send (&(U(i,local_sizes[rank]-1)), 1, MPI_DOUBLE, right_neighbor, 0, MPI_COMM_WORLD);

            MPI_Recv (&(U(i,local_sizes[rank])), 1, MPI_DOUBLE, right_neighbor, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            MPI_Recv (&(U(i,-1)), 1, MPI_DOUBLE,left_neighbor, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
}